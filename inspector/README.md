...
- The Protection tab detects `REC-ENC-v1` and `REC-TOK-v1` values. It can
  ...
  session-memory-only keys. Replay inserts decrypted request values only when
  its separate checkbox is explicitly enabled; redacted and tokenized values
  remain irreversible.
...
when the selected HAR entry changes. They can still be exposed through the
